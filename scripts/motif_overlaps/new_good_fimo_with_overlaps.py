...
try:
    ...
    df = df.sort_values(['chr_name', 'sequence_start'])

    i = 1
    while i < df.shape[0]:
        if df.iloc[i]['chr_name'] == df.iloc[i-1]['chr_name'] and df.iloc[i]['sequence_start'] <= df.iloc[i-1]['sequence_stop']:
            ...
            if not overlap: # If overlap list is empty (previous motif hit was not overlapping the one before it)
                if score_A >= score_B:
                    ...
                    overlap.extend([score_A] * (df.iloc[i-1]['sequence_stop'] - df.iloc[i-1]['sequence_start'] + 1))
                    ...
                    overlap.extend([score_B] * (df.iloc[i]['sequence_stop'] - df.iloc[i]['sequence_start'] + 1))
            else: # If overlap list is not empty (previous motif hit was overlapping the one before it)
                for x in range(len(overlap) - (df.iloc[i-1]['sequence_stop'] - df.iloc[i]['sequence_start'] + 1), len(overlap)):
            ...
            if overlap: # If there had been overlaps, then calculate total overlap score
                df.iloc[i-1,df.columns.get_loc('score')] = sum(overlap)/len(overlap) # Score is average of list
...
if new_fimo:
    ...
    new_fimo_df.to_csv(os.path.join(args.output_path, f"{args.tf_ID}.fimo_with_overlaps_no_strand.txt"), sep="\t", index=False)
