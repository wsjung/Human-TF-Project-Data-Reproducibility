...
for master_file in sorted(os.listdir(os.path.join(args.master_path, eqtl_tissue))):
    ...
    df_master = df_master.set_index(['TF','GENE'])
...
output_path = os.path.join(args.output_path, "master_network_per_eqtl_tissue", eqtl_tissue)
os.makedirs(output_path, exist_ok=True)
df_master_final.to_csv(os.path.join(output_path, "master.tsv"), sep="\t", index=False)
